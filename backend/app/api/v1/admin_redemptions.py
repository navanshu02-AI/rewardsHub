...
async def get_redemptions(status: Optional[str] = Query(None)) -> List[RewardRedemption]:
...
async def update_redemption(
    ...
    db = await get_database()
    existing = await db.redemptions.find_one({"id": redemption_id})
    if not existing:
    ...
    updated = await db.redemptions.find_one({"id": redemption_id})
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Redemption not found")
